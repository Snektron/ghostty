...
void main() {
    ...
    // gl_VertexIndex. Since we use instanced drawing, we have 4 vertices
    // for each corner of the cell. We can use gl_VertexIndex to determine
    ...
    case MODE_BG:
        ...
        if (grid_coord.y == 0 && padding_vertical_top != 0) {
            cell_pos.y -= grid_padding.r;
            cell_size_scaled.y += grid_padding.r;
        } else if (grid_coord.y == grid_size.y - 1 && padding_vertical_bottom != 0) {
